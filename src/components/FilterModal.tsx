      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden border border-orange-200">
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-180px)]">
          <div className="p-6 space-y-4">
                  ...
                  {categories.map((category) => (
                    <label key={category.id} className="flex items-center cursor-pointer group">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`flex items-center justify-center w-6 h-6 border-2 rounded-lg mr-3 transition-all duration-200 ${
              ...
              {expandedSections.price && (
                <div className="pl-4 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{t.min}</label>
                      <div className="relative">
                        <input
                          type="number"
                          value={filters.priceRange.min}
                          onChange={(e) => handlePriceChange('min', Number(e.target.value))}
                          className="w-full pl-3 pr-12 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                          placeholder="0"
                        />
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{t.max}</label>
                      <div className="relative">
                        <input
                          type="number"
            ...
            <div className="space-y-3">
              <SectionHeader 
                title={t.language === 'kg' ? 'Сатуучу' : 'Продавец'} 
                icon={User} 
                sectionKey="seller" 
              />
              {expandedSections.seller && (
                <div className="pl-4">
                  <input
                    type="text"
                    placeholder={t.language === 'kg' ? 'Сатуучунун аты' : 'Имя продавца'}
                    value={filters.seller}
                    onChange={(e) => setFilters(prev => ({ ...prev, seller: e.target.value }))}
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                  />
                </div>
              )}
            </div>
                  ...
                  {sortOptions.map((option) => (
                    <label key={option.id} className="flex items-center cursor-pointer group">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`flex items-center justify-center w-6 h-6 border-2 rounded-full mr-3 transition-all duration-200 ${
